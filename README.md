...
The solution completely replaces the proprietary Wi-Fi dongle allows monitoring only through the iSolarCloud service. This means the inverter can be properly monitored in an isolated environment, where an internet connection isn't available, or where the security conscious owner doesn't want persistent connections from their network to external/international services through black-box device.
This has been tested on a Sungrow SG8K-D, but should work on all PV Grid-Connected String Inverters listed in the [Sungrow communication protocol documentation](Sungrow - Communication Protocol of PV Grid-Connected String Inverters_V1.1.37_EN.pdf).
